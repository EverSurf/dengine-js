...
const binariesSource =
  process.env.TON_CLIENT_BIN_SRC ||
...
async function main() {
  ...
  await dl(
    path.join(binariesTargetPath, `index.js`),
    `dengineweb_${binariesVersion}_wasm_js`
  );
  await dl(
    path.join(binariesTargetPath, `dengineweb.wasm`),
    `dengineweb_${binariesVersion}_wasm`
  );
}
