...
export class TestsRunner {
  ...
  async getGiver(): Promise<Account> {
    ...
    const client = this.client;
    const giverContract = getDefaultGiverContract(
      getEnv("EVERCLOUD_GIVER_TYPE") ?? "v2"
    );
    ...
    if (accounts.length === 0) {
      throw new Error(
        `Giver wallet does not exist. Send some grams to ${await giver.getAddress()}`
      );
    }
    const account = accounts[0];
    if (!account.balance || Number(account.balance) < giverRequestAmount) {
      throw new Error(
        `Giver has no money. Send some grams to ${await giver.getAddress()}`
      );
    }
  ...
  async getAccount(
    ...
    const pkg: ContractPackage | undefined =
      packages[Number(abiVersion) as ABIVersion];
    if (!pkg) {
    ...
    return new Account(
      ...
      abiContract(pkg.abi),
      signer ??
        signerKeys(await this.client.crypto.generate_random_sign_keys()),
      {
        tvc: pkg.tvc,
  ...
  async done() {
    ...
    if (giver) {
      for (const account of this.deployedAccounts) {
        try {
          await account.run("sendAllMoney", {
            dest_addr: await giver.getAddress(),
          });
        } catch (e) {
          // ignore exception
        }
      ...
      await new Promise((resolve) =>
        TestsRunner.setTimeout(resolve as any, 1000)
      );
    }
    if (this.client) {
      this.client.close();
